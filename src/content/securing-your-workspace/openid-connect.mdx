...
<Note variant="note">
  OIDC Provider Settings can only be configured if there's at least one [Service Account](doc:service-accounts) in your organization
</Note>
...
- See [Setup GitHub Actions to authenticate to Cloudsmith using OIDC](doc:setup-cloudsmith-to-authenticate-with-oidc-in-github-actions) for a step-by-step guide.
