...
Cloudsmith retention rules automate storage management by deleting packages based on different criterias:
...
<Note>📘 Although a re-sync process will re-evaluate retention rules, it won't alter the upload date on a package.</Note>
...
<Note>📘 From the UI, use the sliders to configure rule values, and finally click the green "Update" button to apply it.
