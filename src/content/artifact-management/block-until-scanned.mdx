...
# Block Until Scanned (BUS)

**Block Until Scan (BUS)** is an optional, advanced security feature designed to significantly enhance the integrity and security of software packages served by Cloudsmith. This feature mandates that all relevant security and compliance policy checks:
- license
- vulnerability
- and package deny policies

are fully completed *before* a package is made available for download.

By enabling BUS, all package requests originating from upstreams configured in "Cache and Proxy" or "Cache Only" mode are subjected to a mandatory scanning and policy evaluation process. This ensures that only packages meeting your organization's predefined security and compliance requirements can be accessed by users, thereby mitigating the risk of distributing unverified or non-compliant software.

### How It Works:

* **Behavior With BUS Enabled**:
    With BUS enabled, any package fetched directly from an upstream source is held and *not* served to the client until all relevant security and compliance policy checks have been successfully completed. This ensures that only fully vetted and compliant packages are ever distributed, preventing the initial download of potentially problematic software.

* **Without BUS enabled**:
    When a client requests a package, Cloudsmith first checks if it's already synchronized and available in the repository. If so, relevant policy checks have usually been performed asynchronously, and any policy-violating packages would be quarantined. However, if the package isn't yet in Cloudsmith and needs to be fetched from an upstream, the package is served to the client *before* all necessary policy checks are completed. This allows for the initial download of packages that may subsequently fail policy checks, potentially introducing vulnerable or non-compliant software into your build environments or client systems.

## How to Use It

### Enabling Block Until Scan

The Block Until Scan feature is currently in **Early Access (EA)**. To enable BUS for your workspace, please contact Cloudsmith Support.

#### Requirements and Considerations:

* **Active Security Policies**: At least one active classic policy with a 'quarantine' action enabled is required. Without such a policy (e.g., an active Vulnerability or License Policy), the system would have no criteria to block against, and thus no delay in responses would occur.
* **Upstream Configuration ('Proxy and Caching' mode)**: the repository must have an upstream configured with 'Proxy and Caching' mode enabled. BUS functionality is explicitly applied to packages undergoing the caching and evaluation pipeline within Cloudsmith.
* **Workspace-Wide Setting**: Please note that BUS is a global setting that affects all repositories within a given workspace. Its activation will impact all applicable package flows.
...
* **Policy Evaluation**: The package is evaluated against all active policies.</Note>
...
1.  **Feature Activation**: Contact Cloudsmith Support to enable the BUS feature for your workspace. Ensure all prerequisites (e.g., active policies) are met.
...
3.  **Package Pull (With BUS Enabled)**:
    Execute a command to pull a new, uncached package from the Cloudsmith CLI (or your native package manager) that is known to trigger a policy violation or simply requires a scan.
...
4.  **Observe Behavior**:
    * **Expected Behavior (With BUS)**: When BUS is enabled, the initial download request for an uncached package will be temporarily blocked until the policy evaluation has completed.
