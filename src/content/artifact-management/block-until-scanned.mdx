...
* **Behavior With Block Until Scan Enabled**:
    With Block Until Scan enabled, any package fetched directly from an upstream source is held and *not* served to the client until all relevant security and compliance policy checks have been successfully completed. This ensures that only fully vetted and compliant packages are ever distributed, preventing the initial download of potentially problematic software.
...
3.  **Package Pull (With Block Until Scan Enabled)**:
