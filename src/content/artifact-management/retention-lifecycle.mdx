...
import overview from './images/retention-lifecycle/overview.png'
...
Resynchronization of packages does not evaluate retention rules.
...
<Note>📘 Note: `retention_package_query_string` is defined in an opposite meaning than the other parameters: it defines the packages to delete, not the packages to retain.</Note>

<Note>📘 About multiple groupings</Note>

### Configuration parameters via API

Visit API reference.

As a reference, use `GET` to consult existing retention rules or (PATCH) to update them.
...
The **Limit by count** option defines the number of packages to keep. For example, if we set this to `4` and only 3 packages meet the criteria, none will be deleted. But if 5 do meet the criteria, then 1 will be deleted and 4 will be keep in the repository.
...
### Deletion order
Packages are deleted based on the date pushed, starting with the oldest. Newest packages will be kept.
