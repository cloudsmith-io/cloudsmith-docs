...
| **Availbility**          | General availability (GA)             | Early access (EA)                               |
...
When your repository has been a private broadcast enabled, you’ll a status indicator on the packages tab. 
...
1. **Generate a Temporary Token:** Use the broadcast-token endpoint to generate a temporary access token. The API requires the secret key from an existing entitlement token as input.
