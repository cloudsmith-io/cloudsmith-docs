...
You can create Standard Entitlement Tokens via the [Cloudsmith web app](/software-distribution/entitlement-tokens-via-the-website-ui),  via the [Cloudsmith CLI](/software-distribution/entitlement-tokens-via-the-cloudsmith-cli) or via the [Cloudsmith API](/software-distribution/entitlement-tokens-via-the-cloudsmith-api) 
...
Only the user themselves and administrators of the repository can see the user-based tokens. If the user is removed from the repository, teams or the workspace, then the token will no longer work for authentication.
...
- You can [sync entitlement tokens](/software-distribution/entitlement-tokens-via-the-cloudsmith-cli#synchronising-tokens) across multiple repositories.
