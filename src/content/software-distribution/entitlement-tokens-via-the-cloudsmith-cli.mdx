...
```shell
cloudsmith ents list WORKSPACE/REPOSITORY
```
...
```shell
cloudsmith ents create WORKSPACE/REPOSITORY --name=TOKEN-NAME
```
...
for I in $(seq 1 10); do
  cloudsmith ents create WORKSPACE/REPOSITORY --name "Customer $I"
done
```
...
```shell
cloudsmith ents ls WORKSPACE/REPOSITORY --show-tokens -F json | jq -r '.data[] | "\(.name) = \(.token)"'
```
...
```shell
cloudsmith ents delete WORKSPACE/REPOSITORY/IDENTIFIER
```
...
```shell
cloudsmith ents refresh WORKSPACE/REPOSITORY/IDENTIFIER
```
...
```shell
cloudsmith ents update WORKSPACE/REPOSITORY/IDENTIFIER --name=TOKEN-NAME --token=TOKEN-STRING
```
...
```shell
cloudsmith ents sync WORKSPACE/TARGET-REPO  SOURCE-REPO
```
...
```shell
cloudsmith ents restrict WORKSPACE/REPOSITORY/IDENTIFIER <restriction-flags>
```
...
```shell
cloudsmith metrics tokens WORKSPACE/REPOSITORY
```
...
```shell
cloudsmith metrics tokens WORKSPACE/REPOSITORY --tokens=IDENTIFIER
```
