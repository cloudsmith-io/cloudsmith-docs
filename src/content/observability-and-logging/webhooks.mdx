...
You can also add a package search query to webhooks. This uses the same [search syntax](doc:search-packages) supported elsewhere for searching. If present, packages that emit events need to match the search query provided. In this way, you could filter webhooks for only certain types of packages, in addition to specific events.
