...
import set_secret from './images/renovate/set_secret.png';
import host_rule from './images/renovate/host_rule.png';
import output1 from './images/renovate/output1.png';
import output2 from './images/renovate/output2.png';

# Renovate

_How to integrate Mend Renovate with Cloudsmith._

---

[Renovate](https://github.com/renovatebot/renovate) is a popular open source dependency update automation tool. It can help you by automating the process of looking for references to dependencies (both public and private) and updating them if there are newer versions available. Renovate supports a wide range of package ecosystems (Docker, npm, PyPI, Maven, NuGet, etc.) and works across multiple version control systems (GitHub, GitLab, Azure DevOps, Bitbucket).

<Note>📘 This guide shows how to configure Renovate to work with a private Cloudsmith Docker repository, using the Renovate GitHub App (hosted by Mend) as the primary example.However, the same approach applies to other package types supported by Cloudsmith, including npm, PyPI, Maven, NuGet, and more.</Note>

## Why use Renovate with Cloudsmith?

- Automatically create pull requests to update your dependencies when new versions are published to Cloudsmith.
...
<Note>📘 We recommend using a Cloudsmith [Entitlement token](/software-distribution/entitlement-tokens) instead of an API key.</Note>

In the next step, you'll configure your Entitlement Token via Renovate’s Host Rules.
...
<BlockImage src={set_secret} alt=""></BlockImage>

### 2. Add a Host Rule

In the Mend Developer Dashboard, navigate to Settings → Host Rules → Add Host Rule and fill the fields:
...
<BlockImage src={host_rule} alt=""></BlockImage>

<Note>📘  You do not need to add a renovate.json file to your repository — the hosted App manages this for you!</Note>
...
<BlockImage src={output1} alt=""></BlockImage>
...
<BlockImage src={output2} alt=""></BlockImage>
