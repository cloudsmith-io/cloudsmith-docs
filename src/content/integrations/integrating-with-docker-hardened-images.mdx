...
# Integrating with Docker Hardened Images (DHI)
...
You can retrieve these images through Cloudsmith by enabling the upstream to DHI.

Docker offers both a free tier registry (`dhi.io`) containing developer images and a DHI Enterprise tier which provides features like FIPS or STIG compliance variants, customization capabilities, or SLA-backed support. DHI Enterprise allows for images to be mirrored to a private organisation. 
...
1. **Configure Upstream Proxying**  
   ...
   - For mirrored DHI Enterprise images, you can pull through both `https://dhi.io` or a standard [DockerHub integration](/integrations/integrating-with-docker-hub) by specifying your organisation name in the image path (mydockerorg/dhi-node:latest).
