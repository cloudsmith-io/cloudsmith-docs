...
If you haven't already done so you'll need to expose your Cloudsmith [API Key](https://cloudsmith.io/user/settings/api/) within Jenkins. We recommend using the [credentials plugin](https://wiki.jenkins-ci.org/display/JENKINS/Credentials+Plugin) then injecting the `CLOUDSMITH_API_KEY` environment variable into your build jobs.
