...
<NoCodeNoteSnippet />
<a target="_blank" href="https://youtu.be/IykYs_VxbG0"><img src="https://files.readme.io/1d8c507-cloudsmith-youtube-play-semaphore-small.png"/></a>
...
You need to add your Cloudsmith [API Key](https://cloudsmith.io/user/settings/api/) in order to authenticate to Cloudsmith for pushing packages. We recommend storing your Cloudsmith API Key as a [secret in Semaphore](https://docs.semaphoreci.com/essentials/using-secrets/). Secrets in Semaphore are organization-level objects that can contain environment variables or files which you can then access in your build jobs. 
...
In the above example, we use the command executed during the block prologue to install the Cloudsmith CLI, so that the `cloudsmith push` command is available.
 ...
 In addition, we also use the `cache restore` command to retrieve the package file that was built in a previous block.  Please see the [Semaphore Cache documentation](https://docs.semaphoreci.com/essentials/caching-dependencies-and-directories/) for more detail on storing and retrieving artifacts using the Semaphore cache
