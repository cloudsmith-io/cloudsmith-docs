...
<Note headline="Custom Note">
  This configuration allows Cloudsmith to authenticate requests from `Some code` Azure DevOps using OIDC,
  provided the token claims match.
</Note>
...
<Note variant="caution" headline="Custom Caution">
  This configuration allows Cloudsmith to authenticate requests from `Some code` Azure DevOps using OIDC,
  provided the token claims match.
</Note>
