...
Cloudsmith provides a native GitHub action integration, that avoids a manualexchange of tokens.
...
jobs:
  push:
    ...
    steps:
      ...
      # The step above configures the CLOUDSMITH_API_KEY for subsequent steps.
      - name: Cloudsmith Push Debian Package
        uses: cloudsmith-io/action@v1
        with:
          command: 'push'
          format: 'deb'
          owner: 'WORKSPACE'             # Your Cloudsmith account name (namespace)
