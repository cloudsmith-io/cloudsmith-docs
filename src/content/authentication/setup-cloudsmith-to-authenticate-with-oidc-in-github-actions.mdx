...
2. Set Access Controls
   1. Configure the repository access controls to allow the [Service Account ](/repositories/repository-privileges#access-control)to push packages to Cloudsmith.
    ...
    steps:
      ...
      - name: Get OIDC token
        run: |
          ...
          curl --request GET --url https://api.cloudsmith.io/user/self/ --header "X-Api-Key:Bearer $token" --header 'accept: application/json'
