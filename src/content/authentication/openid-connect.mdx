...
<Note variant="note">
  OIDC Provider Settings can only be configured if there's at least one [Service Account](/teams-and-accounts/service-accounts) in your organization
</Note>
...
For further details on configuring Github Actions for OIDC see the guide [here](/authentication/setup-cloudsmith-to-authenticate-with-oidc-in-github-actions).
...
This token will also work with the [Command-Line Interface](/getting-started/cli) when specified as `CLOUDSMITH_API_KEY`.
