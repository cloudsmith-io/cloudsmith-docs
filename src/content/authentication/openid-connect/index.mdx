...
import imge0b23d4_Group_2554 from './../images/openid-connect/e0b23d4-Group_2554.png'
import img_annotated_openidconnect from './../images/openid-connect/2065557e5fdb762669dd32fa648efab1def4ff7a918e3e728674ca97099f6a04-annotated-openidconnect.png'
import img_provider_settings from './../images/openid-connect/e031ac566d5d50d46982aebe44f8b30ce6ce4fba647aad0678ac3466414dde82-provider-settings.png'
...
Cloudsmith supports [OpenID Connect (OIDC)](https://openid.net/connect/) authentication to allow users to authenticate to Cloudsmith programmatically without needing to manage long-lived API keys or tokens. Additionally, OIDC allows for fine-grained access control by mapping claims from the OIDC provider to specific Cloudsmith service accounts using dynamic claim mapping.

OpenID Connect can be configured via:
- The Cloudsmith web app.
- The Cloudsmith API.
- The Terraform provider.

While this is not an exhaustive list, Cloudsmith has tested and verified OIDC integration with the following providers:
...
- [GitLab](https://docs.gitlab.com/ee/ci/yaml/index.html#id_tokens)
- [Bitbucket Pipelines](https://support.atlassian.com/bitbucket-cloud/docs/integrate-pipelines-with-resource-servers-using-oidc/)

Follow the links above for provider-specific documentation and examples.
...
| **Mapping Claim**                  | Specifies which claim will be used when dynamic mapping of OIDC claims to service accounts is enabled.        |
| **Claim to Service Mapping**       | Defines the mapping of claim values to service account slugs.        |
| **Service Account**                | Non-human identity used to authenticate and authorize access to your Cloudsmith resources and APIs programmatically  |
| **ACCOUNT**                        | Your Cloudsmith Organization slug/identifier                                                                                  |

## How OIDC Works

The following diagram gives an overview of how an OIDC provider integrates with your workflows and Cloudsmith. The next componentsa are involved:
...
- **Auth Server**: the OIDC provider (for example, the GitHub Actions OIDC Server) that issues OpenID Tokens to the **Client**.
- **Client**: the Service requesting authentication (an application running in a worker in GitHub Actions, for example).

Implicit trust between the Service and the Authentication Server is necessary to allow seamless OIDC token exchange and verification.

<BlockImage src={imge0b23d4_Group_2554} alt=""></BlockImage>
...
OIDC Provider Settings for your workspace can be accessed and configured browsing to your workspace settings, and then selecting the **Authentication** option in the left menu. Alternatively, use the next link replacing `WORKSPACE` with your workspace name:

`https://app.cloudsmith.com/{WORKSPACE}/settings/authentication/openid` 

<BlockImage src={img_annotated_openidconnect} alt=""></BlockImage>

<Note variant="note" headline="Permissions required">
...
<BlockImage src={img_provider_settings} alt=""></BlockImage>

<Note variant="note" headline="Service Accounts required">
  OIDC Provider Settings can only be configured if there's at least one [Service Account](/accounts-and-teams/service-accounts) in your organization.
</Note>
...
|**Provider URL**|A Provider URL. This is unique to each provider, for example for Github Actions it is: `https://token.actions.githubusercontent.com` **Note: this needs to be the root URL i.e. it has the `.well-known/openid-configuration` portion of the URL removed**.<br /><br />This provides the configuration for the chosen provider; we use this to verify the provider signed the OpenID Token we received|
|**Required OpenID Token Claims**|The required claims must be in the received OpenID Token's payload to authenticate successfully.<br /><br />For example, tokens from CircleCI could specify the [`oidc.circleci.com/vcs-origin`](https://circleci.com/docs/openid-connect-tokens/#format-of-the-openid-connect-id-token) value, which would allow users to limit requests from a specific version control source repository to allow access to specified Cloudsmith Service Account(s). <br /><br />We **strongly** encourage people to set at least one claim, as some providers share signing keys across all orgs, meaning that without any configured claims, any OIDC request from that provider could authenticate if they knew the service account(s) to target.<br /><br />We support adding a wildcard (`.*` ) operator, **only** at the end of the value of the Claims.|
...
<Note variant="note" headline="Dynamic Claim Mapping">
Please, note that configuration of Dynamic Claim Mapping is only available via the API and Terraform provider.
</Note>
...
- See [Setup GitHub Actions to authenticate to Cloudsmith using OIDC](/authentication/setup-cloudsmith-to-authenticate-with-oidc-in-github-actions) for a step-by-step guide.
...
For further details on configuring Github Actions for OIDC see the guide [here](/authentication/setup-cloudsmith-to-authenticate-with-oidc-in-github-actions) or review the [documentation](https://github.com/cloudsmith-io/cloudsmith-cli-action) of the Cloudsmith CLI GitHub Action.
...
This token will also work with the [Command-Line Interface](/developer-tools/cli) when specified as `CLOUDSMITH_API_KEY`.
