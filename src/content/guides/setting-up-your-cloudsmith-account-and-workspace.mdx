...
<Note variant="warning" headline="Todo">
...
<Note variant="note" headline="Package Formats">
  This guide uses the `npm` package manager as an example, but the steps below are applicable for all supported [package formats](/supported-formats). Format-specific instructions are available within the Cloudsmith web app.
</Note>
...
A verification email containing a log in link will be sent to the email address used for account sign up. Open this email and click the link to complete the account sign up process. 
