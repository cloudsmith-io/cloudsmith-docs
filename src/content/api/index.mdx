...
The simplest way to authenticate is to provide an email and an associated token (such as an entitlment token, or API key) when making API requests (replacing `email` with a valid email address and `token` with a valid, associated token):
...
You should treat all tokens as secrets, and they should not be shared. We recommend using an API Key instead and using a lesser privileged account for API access.
