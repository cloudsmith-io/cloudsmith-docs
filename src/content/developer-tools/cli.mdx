...
For example, you can use the Cloudsmith CLI container to push a python package to your workspace/repository `WORKSPACE/REPOSITORY`. In this example, the python package `PACKAGE.whl` is located `/path_to_package/`, and the package is being mounted in the container filesystem in `/tmp/` as `my_package.whl`:
