...
[Docker](https://www.docker.com/) is an ecosystem that is used to run packaged software known as containers. A container image is bundled with all of the software and configured required to run as an independent process (or collection of processes), and an executing container is isolated from other containers and processes.

Docker provides the concept of the _registry_ as a storage service for storing and distributing image containers for use. The official one of which is [Docker Hub](https://hub.docker.com), which is owned and operated by Docker itself.
