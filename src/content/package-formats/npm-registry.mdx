...
You can then [publish the package](/package-formats/npm-registry#upload-a-package) directly using `npm publish`, or generate the package with `npm pack` and then push via the Cloudsmith CLI or Website UI
...
<Note variant="note" headline="Note">
To use `npm audit`, you must authenticate using a Cloudsmith [API-Key](/getting-started/cli#getting-your-api-key), not an [Entitlement Token](/distributing-your-software/entitlement-tokens).
</Note>
