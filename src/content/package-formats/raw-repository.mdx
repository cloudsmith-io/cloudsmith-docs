...
<Note variant="note" headline="Note">
When you upload raw package, you can specify a package name with `--name` via the CLI, or via the Web UI upload form. If you specify a name that exactly matches the package filename, this is treated as if a name was not specified and the resulting [download links](/package-formats/raw-repository#download-via-command-line) will follow the format for a package that was uploaded without a name.
</Note>
