...
<Note variant="note">
  You can view this violation list at any time using our package search [(link)](/artifacts/search-packages#searching-packages-via-the-website-ui) feature and setting the filter to `policy_violated:true` for all policy violations, or `license_policy_violated:true` to only return license policy violations.
</Note>
