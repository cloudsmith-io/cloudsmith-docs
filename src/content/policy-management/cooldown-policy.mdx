...
To create and manage a cooldown policy, you must have access to Policies in your workspace.
...
You can enable an existing cooldown policy:

- From the **Policies** tab of the Cloudsmith dashboard
- From the individual policy detail page 
- In the **Edit policy** view
...
You can also enable a policy from the individual policy view:



### Disable a cooldown policy via the Cloudsmith web app (recommended method)

If a policy is disabled it will also take a few minutes for this change to take effect on packages already in your workspace


### Update a cooldown policy via the Cloudsmith web app (recommended method)

To update an existing cooldown policy, go to **Policies** and select the cooldown policy from the policy list. The policy opens in read-only mode. Click **Edit policy** to make changes.
...
A cooldown policy can also be deleted from this view.
