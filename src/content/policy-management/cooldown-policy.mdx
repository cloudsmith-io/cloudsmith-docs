...
The cooldown policy feature is in early access. To try this feature, please [contact us](https://cloudsmith.com/company/contact-us).</Note>
...
If a cooldown policy is updated, disabled, or deleted, packages will be reevaluated. This is different from other policies that only apply to cached packages and do not have automatic reversal.</Note>
...
<Note variant="tip">
You can exclude upstream packages that are already cached in your Cloudsmith workspace from the policy scope by unchecking the **Apply to packages already in use** option in the policy view.</Note>
...
<Note> When created from a template, your cooldown policy is disabled by default. </Note>
...
<Note> After a cooldown policy is created, it may take several minutes for the non-compliant packages in your Cloudsmith workspace to appear as **HIDDEN**.</Note>
