...
import deny_violation from './images/deny-policy/deny_violation.png'
import search_deny_true from './images/deny-policy/search_deny_true.png'
import audit_policy from './images/deny-policy/audit_policies.png'
...
While deny policies can be applied to Docker images by their **package name** (pull string) or **image digest**, they won't apply to any of their internal components. For example, a deny policy with the next query: `format:python`, won't block Docker images containing Python artifacts within the Software Bill of Materials (SBOM).
</Note>
...
<BlockImage src={deny_violation} alt=""></BlockImage>
