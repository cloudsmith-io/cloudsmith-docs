...
```shell
choco push PACKAGE_NAME-PACKAGE_VERSION.nupkg -Source https://nuget.cloudsmith.io/WORKSPACE/REPOSITORY/v2 -ApiKey API-KEY
```
...
choco source add -n example-repo -s https://nuget.cloudsmith.io/WORKSPACE/REPOSITORY/v2 -u "token" -p "TOKEN"
```
```shell Username & Password
choco source add -n example-repo -s https://nuget.cloudsmith.io/WORKSPACE/REPOSITORY/v2 -u "USERNAME" -p "PASSWORD"
```
```shell Username & API-Key
choco sources add -n example-repo -s https://nuget.cloudsmith.io/WORKSPACE/REPOSITORY/v2 -u "USERNAME" -p "API-KEY"
```
