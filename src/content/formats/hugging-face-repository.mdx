...
```bash
hf upload <MODEL_NAME> <LOCAL_PATH_TO_MODEL> --repo-type=<TYPE> --commit-message="Commit via CLI" --revision=<VERSION>
```

For example, to upload a model called `microsoft/kosmos-2.5` stored locally at `./hugging-face-models/microsoft/kosmos-2.5`:

```bash
hf upload microsoft/kosmos-2.5 ./hugging-face-models/microsoft/kosmos-2.5 --repo-type=model --commit-message="Commit via CLI" --revision="1.2.3"
```
...
```bash
hf download <MODEL_NAME> --repo-type <TYPE> --revision <VERSION>
```

For example, to download version `4.5.6` of the `microsoft/kosmos-2.5` model:

```bash
hf download microsoft/kosmos-2.5 --repo-type model --revision 4.5.6
```
