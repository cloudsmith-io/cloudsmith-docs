...
```
https://nuget.cloudsmith.io/WORKSPACE/REPOSITORY/v2
```
...
```powershell
Register-PSRepository -Name 'NAME' -SourceLocation 'https://nuget.cloudsmith.io/WORKSPACE/REPOSITORY/v2/' -PublishLocation 'https://nuget.cloudsmith.io/WORKSPACE/REPOSITORY/v2/' -InstallationPolicy 'trusted' -Credential $credential
```
...
```shell
cloudsmith push nuget WORKSPACE/REPOSITORY MODULE_NAME-MODULE_VERSION.nupkg
```
...
```powershell
Register-PackageSource -Name 'NAME' -Location 'https://nuget.cloudsmith.io/WORKSPACE/REPOSITORY/v2/' -Trusted -ProviderName NuGet
```

```powershell
Register-PSRepository -Name 'NAME' -SourceLocation 'https://nuget.cloudsmith.io/WORKSPACE/REPOSITORY/v2/' -InstallationPolicy 'trusted'
```
...
Register-PackageSource -Name 'NAME' -Location 'https://nuget.cloudsmith.io/WORKSPACE/REPOSITORY/v2/' -Trusted -ProviderName NuGet -Credential $credential

Register-PSRepository -Name 'NAME' -SourceLocation 'https://nuget.cloudsmith.io/WORKSPACE/REPOSITORY/v2/' -InstallationPolicy 'trusted' -Credential $credential
```
