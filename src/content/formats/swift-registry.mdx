...
In the next steps we cover how to package an existing project from GitHub and upload it to your Cloudsmith repository.

For this example, we'll be using **Alamofire**: a popular HTTP networking library written in Swift.

Download its source code and checkout version `5.9.1`:

```bash
cd .. #get back to the previous root dir
...
You are ready to publish the package to your Cloudsmith Registry. Follow the previous steps to configure and login to your registry (replace `WORKSPACE` and `REPOSITORY` with your own):
...
To upload via the Cloudsmith CLI / API, you'll need to generate your package first. We can use the first example to test it:
...
Create a new directory for a sample project and `cd` into it.
...
### Configure the Registry for the New Project

Just as before, set the registry URL and log in.

```bash
swift package-registry set "https://swift.cloudsmith.io/WORKSPACE/REPOSITORY/"
swift package-registry login "https://swift.cloudsmith.io/WORKSPACE/REPOSITORY/" --username USER --password API_TOKEN
```
