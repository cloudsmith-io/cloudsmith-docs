...
HTTP Basic Auth with credentials in the userinfo portion of the URL (e.g., `token:TOKEN@dl.cloudsmith.io`) is supported on this domain when using the appropriate path.
...
| `dl.cloudsmith.io` | Path-based/Basic Auth* |
* Not all forms of Basic Auth are supported on this domain. See the documented example, HTTP Basic Auth with credentials in the userinfo portion of the URL.
