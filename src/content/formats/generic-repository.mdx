...
To use either an entitlement token or an API key to download files directly:
...
Or use the token in the URL path:

```shell
curl -sLf -O "https://generic.cloudsmith.io/TOKEN/OWNER/REPOSITORY/UPSTREAM_PREFIX/path/to/file.tar.gz"
```

<Note variant="note" headline="Browser limitations">
The URL formats above work for direct file downloads via command line tools. However, they do not work when browsing the HTML index in a web browser. For browser access, use HTTP Basic authentication when prompted (enter `token` as the username and your entitlement token as the password).
</Note>

**For HTTP libraries that don't follow redirects:**

Some HTTP libraries do not automatically follow redirects. For these cases, It's recommend to configure the ability to follow redirects however if that is not possible, use the `dl.cloudsmith.io` domain with the token in the URL path:

```shell
curl -sLf -O "https://dl.cloudsmith.io/TOKEN/OWNER/REPOSITORY/generic/UPSTREAM_PREFIX/path/to/file.tar.gz"
```
...
To view the HTML index in a browser for private repositories, use `token` as the username and your entitlement token or API key as the password when prompted for authentication.
