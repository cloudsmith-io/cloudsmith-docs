...
```
https://npm.cloudsmith.io/WORKSPACE/REPOSITORY/
```
...
```shell NPM \< v9
npm login --registry=https://npm.cloudsmith.io/WORKSPACE/REPOSITORY/
Username: USERNAME
Password: API-KEY
Email: YOUR-EMAIL-ADDRESS
```
```shell NPM v9 +
npm login --auth-type=legacy --registry=https://npm.cloudsmith.io/WORKSPACE/REPOSITORY/
Username: USERNAME
Password: API-KEY
Email: YOUR-EMAIL-ADDRESS
```
...
```
registry=https://npm.cloudsmith.io/WORKSPACE/REPOSITORY/
//npm.cloudsmith.io/WORKSPACE/REPOSITORY/:_authToken=API-KEY
```
...
```shell
npm publish --registry=https://npm.cloudsmith.io/WORKSPACE/REPOSITORY/
```
...
<Note variant="note" headline="Note">
This assumes that you've created a `package.json` file for your project. Please see the official npmjs [package.json reference](https://docs.npmjs.com/files/package.json) (external link) for more information.
</Note>

The command to upload an npm package via the Cloudsmith CLI is: 

```shell
cloudsmith push npm WORKSPACE/REPOSITORY PACKAGE_NAME-PACKAGE_VERSION.tgz
```
...
You can then [publish the package](/formats/npm-registry#upload-a-package) directly using `npm publish`, or generate the package with `npm pack` and then push via the Cloudsmith CLI or Cloudsmith web app
...
```shell
npm dist-tags ls PACKAGE_NAME --registry=https://npm.cloudsmith.io/WORKSPACE/REPOSITORY/
```
...
```shell
npm dist-tags rm PACKAGE_NAME TAG --registry=https://npm.cloudsmith.io/WORKSPACE/REPOSITORY/
```
...
```shell
npm config set registry https://npm.cloudsmith.io/WORKSPACE/REPOSITORY/
```
...
```shell
npm install PACKAGE_NAME --registry=https://npm.cloudsmith.io/WORKSPACE/REPOSITORY/
```
...
registry=https://npm.cloudsmith.io/WORKSPACE/REPOSITORY/
//npm.cloudsmith.io/WORKSPACE/REPOSITORY/:_authToken=TOKEN
```
```shell HTTP Basic Auth (User & Pass)
registry=https://npm.cloudsmith.io/WORKSPACE/REPOSITORY/
//npm.cloudsmith.io/WORKSPACE/REPOSITORY/:username=USERNAME
//npm.cloudsmith.io/WORKSPACE/REPOSITORY/:_password=PASSWORD
```
...
registry=https://npm.cloudsmith.io/WORKSPACE/REPOSITORY/
//npm.cloudsmith.io/WORKSPACE/REPOSITORY/:username=token
//npm.cloudsmith.io/WORKSPACE/REPOSITORY/:_password=TOKEN
```
...
```shell
npm install PACKAGE_NAME@TAG --registry=https://npm.cloudsmith.io/WORKSPACE/REPOSITORY/
```
...
```shell
npm config set '@SCOPE:registry' https://npm.cloudsmith.io/WORKSPACE/REPOSITORY/
```

You can also set it directly in your user or project `.npmrc` file:

```
@SCOPE:registry=https://npm.cloudsmith.io/WORKSPACE/REPOSITORY/
```
...
Configurable Proxying Caching. Please see [Upstream Proxying](/repositories/upstreams) for more details.
...
<Note variant="note" headline="Note">
To use `npm audit`, you must authenticate using a Cloudsmith [API-Key](/developer-tools/cli#getting-your-api-key), not an [Entitlement Token](/software-distribution/entitlement-tokens).
</Note>
