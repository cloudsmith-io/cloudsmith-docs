...
export GOPROXY=https://USERNAME:PASSWORD@golang.cloudsmith.io/OWNER/REPOSITORY/
```
```shell
...
```shell
# Entitlement Token Auth
set GOPROXY=https://golang.cloudsmith.io/TOKEN/OWNER/REPOSITORY/go/,https://proxy.golang.org,direct
```
```shell
# HTTP Basic Auth (User & Pass)
set GOPROXY=https://USERNAME:PASSWORD@golang.cloudsmith.io/OWNER/REPOSITORY/
```
```shell
# HTTP Basic Auth (API-Key)
...
```powershell
# Entitlement Token Auth
$env:GOPROXY=https://golang.cloudsmith.io/TOKEN/OWNER/REPOSITORY/go/,https://proxy.golang.org,direct
```
```powershell
# HTTP Basic Auth (User & Pass)
$env:GOPROXY=https://USERNAME:PASSWORD@golang.cloudsmith.io/OWNER/REPOSITORY/
```
```powershell
# HTTP Basic Auth (API-Key)
...
It is recommended to use the environment variable `GONOSUMDB=cloudsmith.io` to skip verification for your Cloudsmith repositories.

You can also define this for specific workspaces/repos: `GONOSUMDB=jack-testing,demo-docs/awesome-repo,cloudsmith-test/acme2`
</Note>

<Note variant="note" headline="About Go modules authentication">
[sum.golang.org](sum.golang.org) provides an auditable checksum database service used by the go command to authenticate modules.
...
Cloudsmith adheres to this recommendations and proxies checksum verification to the global checksum database.
</Note>
...
Cloudsmith supports ecurity scanning for Go modules.
...
Cloudsmith supports [proxy.golang.org](proxy.golang.org) as an Upstream for your Golang projects. 
