...
```
https://dart.cloudsmith.io/WORKSPACE/REPOSITORY/
```
...
```yaml
publish_to: https://dart.cloudsmith.io/WORKSPACE/REPOSITORY/
```

In order to authenticate for native publishing, you'll need run `dart pub token`, as follows:

```shell
echo 'API-KEY' | dart pub token add https://dart.cloudsmith.io/WORKSPACE/REPOSITORY/
Enter secret token:
Requests to "https://dart.cloudsmith.io/WORKSPACE/REPOSITORY/" will now be authenticated using the secret token.
```
...
```shell
cloudsmith push dart WORKSPACE/REPOSITORY PACKAGE_NAME-PACKAGE_VERSION.tgz
```
...
echo 'TOKEN' | dart pub token add https://dart.cloudsmith.io/WORKSPACE/REPOSITORY/
Enter secret token:
Requests to "https://dart.cloudsmith.io/WORKSPACE/REPOSITORY/" will now be authenticated using the secret token.
```
```yaml HTTP Basic Auth (User & Pass)
echo 'PASSWORD' | dart pub token add https://dart.cloudsmith.io/WORKSPACE/REPOSITORY/
Enter secret token:
Requests to "https://dart.cloudsmith.io/WORKSPACE/REPOSITORY/" will now be authenticated using the secret token.
```
...
```shell
dart pub add PACKAGE_NAME:PACKAGE_VERSION --hosted-url https://dart.cloudsmith.io/WORKSPACE/REPOSITORY/
...
You can configure upstream Dart repositories that you wish to use for packages that are not available in your Cloudsmith repository.
