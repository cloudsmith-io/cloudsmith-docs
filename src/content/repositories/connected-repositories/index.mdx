...
Connected Repositories is in Early Access (EA) and is currently supported for Maven and Docker. Please reach out to us if you are interested in trying it out or have feedback to share.
</Note>

Connected Repositories allows you to natively link multiple repositories together within a workspace, making packages from all connected repositories available through a single URL. This provides first-class "virtual" repository support (similar to JFrog's Virtual Repositories) without the drawbacks of Cloudsmith-to-Cloudsmith upstream configurations.
...
- Packages are available instantaneously across all connected repositories. Cache invalidation occurs immediately when packages are pushed or updated.
