...
<Note>📘 Continuous Security is currently in Early Access (EA) through Cloudsmith’s Enterprise Policy Management. </Note>
...
<Note>📘 Continuous Security is only available for Cloudsmith workspace where EPM has been enabled.</Note>
...
Continuous Security is available for [all package formats](./vulnerability-scanning) currently supported by Cloudsmith’s Vulnerability Scanning feature.