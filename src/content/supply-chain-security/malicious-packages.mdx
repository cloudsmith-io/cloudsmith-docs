...
Malicious packages are software components published to public registries like npm, PyPI, or RubyGems, that have been **intentionally crafted by an attacker to cause harm**. For example, a PyPi package named [django-log-tracker](https://thehackernews.com/2024/02/dormant-pypi-package-compromised-to.html) was compromised in 2024 after 2 years without new releases. When developers unknowingly include these packages in their applications, they open the door to data theft, ransomware, and other damaging attacks for anyone using the affected artifacts.

Another example of a malicious package named `ctx` was discovered on the PyPI (Python Package Index) repository. It mimicked a popular but previously removed package. The malicious version was designed to steal developers' environment variables, which often contain sensitive credentials and API keys.
...
Malicious packages information is available via [Enterprise Policy Management](/supply-chain-security/epm) and [Continuous Security](/supply-chain-security/continuous-security) features (both in Early Access). This means, you can create your own policies to take action when a malicious package is detected.
...
<Note variant="note" headline="Cached packages">
Due to the early access nature of this feature, the malicious package detection is only available for packages that are uploaded to Cloudsmith. This means that cached packages won't be matched against new updates in the malicious packages feed.
</Note>

### Example Rego policy

Creating a policy in Cloudsmith that flags any artifact considered malicious is as simple as checking the `vulnerability.id` field and checking if it starts with the prefix `MAL-`, as all malicious packages are identified with this string. Check one example policy below:
