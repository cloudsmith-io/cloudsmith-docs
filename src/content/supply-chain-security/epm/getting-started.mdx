...
When a package is scanned or triggers the policy, EPM creates a decision log entry. Logs are accessed via the decision logs API, which provides two endpoints: a list endpoint returning lightweight summaries, and a retrieve endpoint for fetching the full decision log detail.
...
Each summary in the list includes: `id`, `started_at`, `ended_at`, `match`, `package_name`, `package_format`, `policy_name`, and an `actions` summary dict.
...
The full log additionally provides:

- `policy_input`: The exact data used to evaluate the policy.
- `policy_output`: The results (match or not, partial rule states).
- `actions`: The full detail of actions invoked against the package.
- `correlation_id`: Groups related evaluations together.
