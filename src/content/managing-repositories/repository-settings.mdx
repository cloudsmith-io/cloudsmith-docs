...
You assign a permission level to a Team or User when you grant them access to the repository (see [Access Controls](doc:access-controls) for information on granting access to a repository), and then you can set precisely what actions that permission level can perform using the repository privileges
...
Then in the repository [Access Controls](doc:access-controls), you would set the Team or User to only have **Read** permissions on the repository.  As a result, they would be able to set or use Entitlement Tokens, but would not have permission for any other repository actions.
