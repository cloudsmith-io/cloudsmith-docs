...
import event_details from './images/client-logs/client_logs_event_detail.png'
import overview from './images/client-logs/client_logs_overview.gif'
import overview_static from './images/client-logs/client_logs_overview_static.png'
import event_detail from './images/client-logs/client_logs_event_detail.png'
import timegraph_filter from './images/client-logs/client_logs_timegraph_filter.gif'
...
<BlockImage src={overview} alt="Client Logs Overview"></BlockImage>
...
- Understand usage of different repository types; private repos, OSS repos, public broadcasts, private broadcasts etc.

## Requirements

To access Client Logs, you should be a *Manager* or an *Owner* of your Workspace.

<Note variant="note" headline="About error logs">
For customers using custom domains, client logs will show both successful and unsuccessful requests. For customers who aren't using custom domains, client logs only contain successful requests.
</Note>
...
<BlockImage src={overview_static} alt="Menus"></BlockImage>
...
| Log details | A list of events result from applying the filters to the search. Here you can visualize every single package downloaded from your workspace. You can review a detailed sample entry [here](/logs-and-observability/client-logs#analyzing-a-specific-entry). |
...
| :--------------------- | :---------- | :--------------------- | :---------- |
| **Target** || **Actor**  |
| Package Format | _Maven_, _Python_, _Docker_... you name it. Cloudsmith supports 30 different [formats](/formats). | IP Address | IP address of the download request | User Type | User or Service. |
| Repository Type | Public, Private, or OSS. | Users | Name, determined by the owner of the _API Token_ used. |
| Repository | the name of your repo. | Entitlement Token | Name of the service or non-human agent that downloaded the package. |

<Note variant="note" headline="About search limits">
  Some of these fields (for example, IP Address, Users and Entitlement Token) could potentially return many thousands of results in a dropdown. A 1k limit is applied, displaying the first 1k results ordered by descending most results.
</Note>
...
<BlockImage src={timegraph_filter} alt="Filtering events in time"></BlockImage>
...
Use the left menu to select your desired time scope: _last 30 minutes_, _last hour_, _last day_, _last year_, or a _custom time range_ for more specific periods.
...
| Target EULA      | If requires acceptance of a End-User License Agreement (_EULA_). |
...
<BlockImage src={event_detail} alt="Filtering events in time"></BlockImage>
...
<Note variant="note" headline="Log entries limit">
  There is a limit of 500 log entries for download operations.
</Note>
