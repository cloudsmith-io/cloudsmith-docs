...
You can also add a package search query to webhooks. This uses the same [search syntax](/artifact-management/search-packages) supported elsewhere for searching. If present, packages that emit events need to match the search query provided. In this way, you could filter webhooks for only certain types of packages, in addition to specific events.
...
These are not guaranteed to remain static forever. For an updated list, please [contact us today](/contact-us)!