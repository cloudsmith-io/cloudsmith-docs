...
There is more information about Cloudsmith's Maven repositories [here](/supported-formats/maven-repository)

## Prerequisites

- Maven artifacts exported to a folder. Follow the steps [here](doc:exporting-1) to export your packages. 
- Create a repository on Cloudsmith using the instructions found [here](doc:create-a-repository) 

## Update settings.xml

Maven is configured using a settings.xml file located under your Maven home directory (typically, this will be /user.home/.m2/settings.xml). Follow the instructions [here](/supported-formats/maven-repository#upload-via-maven) to update your settings.xml file for Maven.
